-- ============================================
-- Anvil Hub — Multi Loader (compact)
-- ============================================
...
if getgenv().ANVIL_LOADER_ACTIVE then return end
getgenv().ANVIL_LOADER_ACTIVE = true

local already = false
pcall(function()
    local pg = player:FindFirstChild("PlayerGui")
    if not pg then pg = player:WaitForChild("PlayerGui") end
    if pg:FindFirstChild("AnvilHubBoot") then
        already = true
    else
        local boot = Instance.new("ScreenGui")
        boot.Name = "AnvilHubBoot"
...
local function forceCloseKeyUI()
...
-- ===== CONFIG =====
local GAMES = {
    [93978595733734] = "vd.lua",       -- Violence District
    [142823291] = "mm2.lua",           -- Murder Mystery 2
    [9872472334] = "evade.lua",        -- Evade
    [17625359962] = "rivals.lua",      -- Rivals
    [95082159892680] = "speedkey.lua", -- Speed Keyboard Escape
    [109983668079237] = "brainrot.lua",-- Steal a Brainrot
    [136020512003847] = "rp.lua", -- San Diego Border Roleplay
}
local BASE_URL = "https://raw.githubusercontent.com/sh1ldonov/anvil_hub/main/"
local scriptFile = GAMES[game.PlaceId]

-- fallback по имени плейса (если PlaceId не совпал, но игра та самая)
if not scriptFile then
    local placeName = string.lower(game.Name or "")
    if placeName:find("san diego") or placeName:find("border roleplay") then
        scriptFile = "rp.lua"
    end
end

-- ===== LOADER UI (compact box) =====
local loadGui = Instance.new("ScreenGui"); loadGui.Name = "AnvilLoader"; loadGui.ResetOnSpawn = false; loadGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling; loadGui.IgnoreGuiInset = true
loadGui.Parent = player:WaitForChild("PlayerGui")
loadGui.Enabled = false

-- accent pulse (синий -> розовый), как в rp.lua
local ACC_P = { r = 60, g = 140, b = 255 }
local ACC_S = { r = 255, g = 70, b = 150 }
local function getAccent()
    local k = (math.sin(os.clock() * 2) + 1) / 2
    return Color3.fromRGB(
        math.floor(ACC_P.r + (ACC_S.r - ACC_P.r) * k),
        math.floor(ACC_P.g + (ACC_S.g - ACC_P.g) * k),
        math.floor(ACC_P.b + (ACC_S.b - ACC_P.b) * k))
end

local box = Instance.new("Frame"); box.Size = UDim2.new(0,200,0,200); box.Position = UDim2.new(0.5,-100,0.5,-100); box.BackgroundColor3 = Color3.fromRGB(9,9,14); box.BorderSizePixel = 0; box.Parent = loadGui
Instance.new("UICorner", box).CornerRadius = UDim.new(0,16)
local bStroke = Instance.new("UIStroke", box); bStroke.Color = getAccent(); bStroke.Thickness = 2; bStroke.Transparency = 0.65

-- game thumbnail (картинка плейса, куда инжектится скрипт)
local thumb = Instance.new("ImageLabel"); thumb.Size = UDim2.new(1,-10,1,-10); thumb.Position = UDim2.new(0,5,0,5); thumb.BackgroundColor3 = Color3.fromRGB(19,19,26); thumb.BorderSizePixel = 0; thumb.Image = ""; thumb.ZIndex = 1; thumb.Parent = box
Instance.new("UICorner", thumb).CornerRadius = UDim.new(0,12)

local thumbTitle = Instance.new("TextLabel"); thumbTitle.Size = UDim2.new(1,0,0,20); thumbTitle.Position = UDim2.new(0,0,0,6); thumbTitle.BackgroundTransparency = 1; thumbTitle.Text = "Anvil Hub"; thumbTitle.TextColor3 = Color3.fromRGB(226,226,238); thumbTitle.Font = Enum.Font.GothamBold; thumbTitle.TextSize = 13; thumbTitle.TextStrokeTransparency = 0.6; thumbTitle.ZIndex = 2; thumbTitle.Parent = thumb

-- loading ring (акцент)
local ring = Instance.new("ImageLabel"); ring.Size = UDim2.new(0,44,0,44); ring.Position = UDim2.new(0.5,-22,0.5,-22); ring.BackgroundTransparency = 1; ring.Image = "rbxassetid://6034501237"; ring.ImageColor3 = getAccent(); ring.ZIndex = 3; ring.Parent = box

-- status text (внутри квадрата)
local statusText = Instance.new("TextLabel"); statusText.Size = UDim2.new(1,-16,0,16); statusText.Position = UDim2.new(0,8,1,-40); statusText.BackgroundTransparency = 1
statusText.Text = ""; statusText.TextColor3 = Color3.fromRGB(128,128,152); statusText.Font = Enum.Font.Gotham; statusText.TextSize = 11; statusText.TextStrokeTransparency = 0.95; statusText.ZIndex = 2; statusText.Parent = box

-- прогресс-бар снизу внутри квадрата
local barBg = Instance.new("Frame"); barBg.Size = UDim2.new(0,120,0,4); barBg.Position = UDim2.new(0.5,-60,1,-22); barBg.BackgroundColor3 = Color3.fromRGB(38,38,52); barBg.BorderSizePixel = 0; barBg.ZIndex = 2; barBg.Parent = box
Instance.new("UICorner", barBg).CornerRadius = UDim.new(1,0)
local barFill = Instance.new("Frame"); barFill.Size = UDim2.new(0,0,1,0); barFill.BackgroundColor3 = getAccent(); barFill.BorderSizePixel = 0; barFill.Parent = barBg
...
local function pulse()
    local a = getAccent()
    pcall(function() bStroke.Color = a end)
    pcall(function() ring.ImageColor3 = a end)
    pcall(function() barFill.BackgroundColor3 = a end)
end
local accCon = rs.RenderStepped:Connect(function(dt)
    accTick = accTick + dt
    if accTick < 0.08 then return end
    accTick = 0
    pulse()
end)

-- ===== LOAD GAME THUMBNAIL (async, no freeze) =====
task.spawn(function()
    local applied = false
    local function apply(uri)
        if not applied and uri and uri ~= "" and uri ~= "0" then
            thumb.Image = uri
            applied = true
        end
    end
    pcall(function() -- client API (returns rbxthumb:// uri)
        local th = game:GetService("Thumbnails")
        apply(th:GetPlaceThumbnail(game.PlaceId, Enum.ThumbnailSize.Size768x432))
    end)
    pcall(function() -- direct rbxthumb uri
        apply("rbxthumb://type=Place&id=" .. game.PlaceId .. "&w=768&h=432")
    end)
    pcall(function() -- fallback: place icon via marketplace
        local info = game:GetService("MarketplaceService"):GetProductInfo(game.PlaceId)
        if info then
            apply("rbxassetid://" .. tostring(info.IconImageAssetId))
        end
    end)
    if not applied then thumb.Image = "" end
end)

-- ===== NOT SUPPORTED GAME — KICK =====
if not scriptFile then
    statusText.Text = "Unsupported game (PlaceId: " .. tostring(game.PlaceId) .. ")"
    pcall(function() accCon:Disconnect() end)
    thumb.BackgroundColor3 = Color3.fromRGB(255,50,50)
    bStroke.Color = Color3.fromRGB(224,80,80)
    ring.ImageColor3 = Color3.fromRGB(224,80,80)
    barFill.BackgroundColor3 = Color3.fromRGB(224,80,80)
    task.wait(2.5)
    game:Shutdown()
    return
end
...
-- ===== SPINNER =====
local spinCon = game:GetService("RunService").Heartbeat:Connect(function(dt)
    ring.Rotation = (ring.Rotation + dt * 220) % 360
end)

-- ===== FETCH IN BACKGROUND (no freeze) =====
local fetchedOk, fetchedCode = false, nil
task.spawn(function()
    local ok, res = pcall(function()
        return game:HttpGet(BASE_URL .. scriptFile)
    end)
    fetchedOk = ok
    fetchedCode = ok and res or nil
end)

-- wait for fetch (bounded) happens after key check; fetch spawns in background above
task.wait(0.15)

-- ===== KEY VERIFICATION (Junkie SDK) =====
local keyResult = { done = false, ok = false, key = nil, err = nil }
task.spawn(function()
    local ok, errmsg = pcall(function()
        ...
        local httpSvc = game:GetService("HttpService")
            ...
            local function getBody(r)
                if not r then return nil end
                if type(r) == "string" then return r end
                for _, k in ipairs({"Body", "body", "Content", "content"}) do
                    local v = r[k]
                    if type(v) == "string" and #v > 0 then return v end
                end
                return nil
            end
            local enc = pcall(function() return httpSvc:UrlEncode(url) end) and httpSvc:UrlEncode(url) or url
            local body = nil
            local attempts = {
                function() return getBody(request({ Url = url, Method = "GET" })) end,
                function() return getBody(http_request({ Url = url, Method = "GET" })) end,
                function() return getBody(syn.request({ Url = url, Method = "GET" })) end,
                function() return game:HttpGetAsync(url) end,
                function() return game:HttpGet(url, true) end,
                function() return game:HttpGet("https://api.allorigins.win/raw?url=" .. enc) end,
                function() return game:HttpGet("https://corsproxy.io/?url=" .. enc) end,
            }
            for _, fn in ipairs(attempts) do
                local ok, res = pcall(fn)
                if ok and res and res ~= "" then body = res; break end
            end
            return body
        end
        task.spawn(function()
            local src = fetchUrl("https://jnkie.com/sdk/library.lua")
            if not src then
                return
            end
            local _ok, chunk = pcall(loadstring, src)
            if not _ok or not chunk then
                return
            end
            local _ok2, lib = pcall(chunk)
            if not _ok2 then
                return
            end
            if not lib then
                local g = nil
                pcall(function() g = getgenv() end)
                if type(g) == "table" and type(g.Junkie) == "table" then lib = g.Junkie end
            end
            if not lib and type(_G.Junkie) == "table" then lib = _G.Junkie end
            if not lib and type(shared) == "table" and type(shared.Junkie) == "table" then lib = shared.Junkie end
            if not lib then
                return
            end
            lib.service = "anvil"
            lib.identifier = "1182379"
            lib.provider = "anvilhub"
            Junkie = lib
        end)

        local options = {
            title = "ANVIL HUB",
            subtitle = "San Diego Border Roleplay",
            description = "Пройдите верификацию ключа, чтобы продолжить"
        }
    ...
    function UI:handleGetLink()
        if not Junkie or not Junkie.get_key_link then
            self:updateStatus("Система ещё не готова, попробуйте позже", Colors.error, 3)
            return
        end
    ...
    function UI:handleVerifyKey()
        ...
        local sdkErr = false
        if Junkie then
            local _okr, _res = pcall(Junkie.check_key, key)
            if not _okr then
                sdkErr = true
            else
                result = _res or nil
        ...
        if result and result.valid then
            ...
            if not Junkie then
                self:updateStatus("Сервер недоступен", Colors.error, 3)
            elseif sdkErr then
                self:updateStatus("Ошибка сервера", Colors.error, 3)
            else
    ...
    local sdkT = 0
    while not Junkie and sdkT < 10 do
        task.wait(0.1)
        sdkT = sdkT + 0.1
    end
    ...
    local result = nil
    if Junkie and keyToCheck ~= "" then
        local _okr, _res = pcall(Junkie.check_key, keyToCheck)
        result = _okr and _res or nil
    end
    if result and result.valid then
    ...
    keyResult.ok = ok
    keyResult.err = errmsg
    keyResult.done = true
end)

-- ===== WAIT FOR KEY SYSTEM RESULT (bounded) =====
statusText.Text = "Проверка ключа..."
local kw = 0
while not keyResult.done and kw < 300 do
    task.wait(0.1)
    kw = kw + 0.1
end

if not keyResult.done or not keyResult.ok then
    loadGui.Enabled = true
    thumb.BackgroundColor3 = Color3.fromRGB(255,50,50)
    bStroke.Color = Color3.fromRGB(224,80,80)
    ring.ImageColor3 = Color3.fromRGB(224,80,80)
    barFill.BackgroundColor3 = Color3.fromRGB(224,80,80)
    statusText.Text = "Ошибка ключа"
...
if not keyResult.key or keyResult.key == "" then
    loadGui:Destroy()
    forceCloseKeyUI()
    return
end

-- ===== SHOW LOADER (после проверки ключа) =====
loadGui.Enabled = true
statusText.Text = "Проверка ключа..."
local steps = {
    {text = "Проверка ключа...", pct = 30},
    {text = "Подготовка...", pct = 55},
    {text = "Загрузка скрипта...", pct = 80},
    {text = "Готово!", pct = 100},
}
for _, step in ipairs(steps) do
    statusText.Text = step.text
    pcall(function() barFill.Size = UDim2.new(step.pct / 100, 0, 1, 0) end)
    ts:Create(ring, TweenInfo.new(0.6, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Rotation = ring.Rotation + 130}):Play()
    task.wait(0.6)
end

-- ===== WAIT FOR FETCH (bounded) =====
statusText.Text = "Загрузка скрипта..."
local kw2 = 0
while not fetchedOk and kw2 < 15 do
    task.wait(0.1)
    kw2 = kw2 + 0.1
end
if not fetchedOk or not fetchedCode then
    loadGui:Destroy()
    forceCloseKeyUI()
    return
end
...
ts:Create(box, fo, {BackgroundTransparency = 1}):Play()
ts:Create(bStroke, fo, {Transparency = 1}):Play()
ts:Create(thumb, fo, {ImageTransparency = 1}):Play()
ts:Create(thumbTitle, fo, {TextTransparency = 1; TextStrokeTransparency = 1}):Play()
ts:Create(ring, fo, {ImageTransparency = 1}):Play()
ts:Create(statusText, fo, {TextTransparency = 1}):Play()
ts:Create(barBg, fo, {BackgroundTransparency = 1}):Play()
ts:Create(barFill, fo, {Transparency = 1}):Play()
task.wait(0.2)
loadGui:Destroy()
forceCloseKeyUI()
