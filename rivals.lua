...
local function notify(text, color)
    pcall(function()
        ...
        f.BackgroundColor3 = Color3.fromRGB(22,22,22); f.BackgroundTransparency = 1; f.BorderSizePixel = 0; f.Parent = notifGui
        local corner = Instance.new("UICorner", f); corner.CornerRadius = UDim.new(0,6)
        local stroke = Instance.new("UIStroke", f); stroke.Color = Color3.fromRGB(40,40,40); stroke.Thickness = 1; stroke.Transparency = 1
        ...
        local lbl = Instance.new("TextLabel", f); lbl.Size = UDim2.new(1,-20,1,0); lbl.Position = UDim2.new(0,16,0,0)
        lbl.BackgroundTransparency = 1; lbl.TextColor3 = Color3.new(1,1,1); lbl.Text = text; lbl.Font = Enum.Font.Gotham; lbl.TextSize = 15; lbl.TextXAlignment = Enum.TextXAlignment.Left
...
local function updateESP()
    ...
    for _, plr in pairs(players:GetPlayers()) do
        if plr ~= player and plr.Character then
            local c = plr.Character; espColor(c, Color3.fromRGB(255,70,70), espCache)
            if (espShowNames or espShowDistance or espShowHealth) and root then
                local cr = c:FindFirstChild("HumanoidRootPart")
                if cr then
                    local b = espBillboards[c]
                    if not b then
                        b = Instance.new("BillboardGui"); b.Parent = c; b.Size = UDim2.new(0,100,0,30); b.StudsOffset = Vector3.new(0,3,0); b.AlwaysOnTop = true
                        local lbl = Instance.new("TextLabel"); lbl.Parent = b; lbl.Size = UDim2.new(1,0,1,0); lbl.BackgroundTransparency = 1; lbl.TextColor3 = Color3.new(1,1,1); lbl.TextStrokeTransparency = 0.5; lbl.Font = Enum.Font.SourceSansBold; lbl.TextScaled = true; espBillboards[c] = b
                    end
                    ...
                    if lbl then
                        local txt = ""
                        if espShowNames then txt = plr.Name end
                        if espShowHealth then
                            if txt ~= "" then txt = txt .. "\n" end
                            txt = txt .. "❤️ " .. health
                        end
                        if espShowDistance then
                            if txt ~= "" and not txt:find("\n") then txt = txt .. " " end
                            if txt:find("\n") then txt = txt .. " [" .. dist .. "m]" else txt = txt .. "[" .. dist .. "m]" end
                        end
                        lbl.Text = txt
                    end
                end
...
local function updateTracers()
    ...
    for _, plr in pairs(players:GetPlayers()) do
        if plr ~= player and plr.Character then local cr = plr.Character:FindFirstChild("HumanoidRootPart"); if cr then table.insert(targets, {pos = cr.Position, color = Color3.fromRGB(255,70,70), name = plr.Name}) end end
    end
...
local function getNearestTarget()
    ...
    local nearest = nil; local nearestDist = aimbotFov
    for _, plr in pairs(players:GetPlayers()) do
        if plr ~= player and plr.Character then
            ...
            if cr then
                ...
                if sp.Z > 0 then
                    ...
                    if dist < nearestDist then nearestDist = dist; nearest = cr end
                end
            end
        end
    end
...
local function unloadAll()
    isUnloaded = true; espEnabled = false; tracersEnabled = false; aimbotEnabled = false; silentAimEnabled = false; noRecoilEnabled = false; noclipEnabled = false; walkSpeedEnabled = false; jumpPowerEnabled = false; antiAfkEnabled = false; fpsBoostEnabled = false
...
pcall(function()
    ...
    local Window = Rayfield:CreateWindow({Name = "RIVALS", LoadingTitle = "RIVALS", LoadingSubtitle = "by anvil", TabPadding = 8, MenuSide = "Left"})
    ...
    uis.InputBegan:Connect(function(input, gp)
        ...
        if input.KeyCode == speedKey then
            local c = player.Character; if c then local h = c:FindFirstChildOfClass("Humanoid"); if h then local o = h.WalkSpeed; h.WalkSpeed = speedAmount; task.delay(3, function() if h and not isUnloaded then h.WalkSpeed = walkSpeedEnabled and walkSpeed or o end end) end end
        end
    end)
    ...
    visTab:CreateToggle({Name = "ESP Health", CurrentValue = false, Callback = function(v) espShowHealth = v end})
    ...
    aimTab:CreateSlider({Name = "Aimbot FOV", Range = {30, 360}, Increment = 5, CurrentValue = 120, Callback = function(v) aimbotFov = v end})
    aimTab:CreateToggle({Name = "Silent Aim", CurrentValue = false, Callback = function(v) silentAimEnabled = v end})
    aimTab:CreateToggle({Name = "No Recoil", CurrentValue = false, Callback = function(v) noRecoilEnabled = v end})

    local moveTab = Window:CreateTab("Movement")
    moveTab:CreateKeybind({Name = "Speed Boost Key", CurrentKeybind = "V", Callback = function(k) speedKey = k end})
    moveTab:CreateSlider({Name = "Boost Speed", Range = {30, 300}, Increment = 5, CurrentValue = 80, Callback = function(v) speedAmount = v end})
    moveTab:CreateToggle({Name = "WalkSpeed", CurrentValue = false, Callback = function(v) walkSpeedEnabled = v end})
    moveTab:CreateSlider({Name = "WalkSpeed", Range = {1, 300}, Increment = 1, CurrentValue = 16, Callback = function(v) walkSpeed = v end})
    moveTab:CreateToggle({Name = "JumpPower", CurrentValue = false, Callback = function(v) jumpPowerEnabled = v end})
    moveTab:CreateSlider({Name = "JumpPower", Range = {0, 300}, Increment = 1, CurrentValue = 50, Callback = function(v) jumpPower = v end})
    moveTab:CreateToggle({Name = "Noclip", CurrentValue = false, Callback = function(v) noclipEnabled = v end})
    moveTab:CreateToggle({Name = "Anti-AFK", CurrentValue = false, Callback = function(v) antiAfkEnabled = v end})
    moveTab:CreateButton({Name = "Unload", Callback = unloadAll})
end)
...
table.insert(connections, rs.RenderStepped:Connect(function()
    ...
    if noRecoilEnabled then
        ...
        if mouse then pcall(function() mouse.Recoil = 0 end) end
    end
end))
